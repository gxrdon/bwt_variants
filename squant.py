...
def parse_file(file_input):
        ...
        for line in f:
            ...
            if count == 0:
                ...
                if len(str(line).split("\\t")) == 1:
                    ...
                    p_2 = 1 / get_effective_length(length)
                    p_2_values.append(p_2)
    ...
    return num_transcripts, transcript_list, alignments, alignment_offsets, precompute, p_2_values
...
#   num_transcripts, transcripts, alignments, alignment_offsets, precompute, p_2_precomputed = parse_file(align_file)
...
@jit(nopython=True)
def equivalence_class_em(align_file, output_file, num_transcripts, transcripts, alignments, alignment_offsets, p_2_precomputed):
...
def call_both_em_functions(align_file, equiv_output, full_output):
    ...
    """
    num_transcripts, transcripts, alignments, alignment_offsets, pre, p_2 = parse_file(align_file)
    full_model_em(align_file, full_output, num_transcripts, transcripts, alignments, alignment_offsets, pre)
    equivalence_class_em(align_file, equiv_output, num_transcripts, transcripts, alignments, alignment_offsets, p_2)
